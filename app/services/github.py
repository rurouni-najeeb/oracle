...
async def _run_gh(args: list[str]) -> str:
    ...
    if proc.returncode != 0:
        logger.warning("gh command failed (exit %d): %s", proc.returncode, stderr.decode().strip())
...
async def fetch_github_data(orgs: list[str]) -> GitHubData:
    ...
    result = GitHubData(review_requests=review_requests, my_prs=my_prs, commented=commented)
...
async def _fetch_review_requests(orgs: list[str]) -> list[PullRequest]:
    ...
    output = await _run_gh([
        "search", "prs", query,
        "--json", "repository,title,number,url,updatedAt,reviewDecision",
        "--limit", "10",
    ])
    if not output.strip():
        return []
...
async def _fetch_commented_prs(orgs: list[str]) -> list[PullRequest]:
    ...
    output = await _run_gh([
        "api", f"search/issues?q={query}&per_page=10&sort=updated&order=desc",
        "--jq", '.items[] | {title: .title, number: .number, url: .html_url, repo: (.repository_url | split("/") | .[-1]), updated: .updated_at}',
    ])
    if not output.strip():
        return []
